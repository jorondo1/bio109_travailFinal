...
library(pacman)
p_load(magrittr, purrr, dplyr, kableExtra)
```
### types de forêt
R: rien 
...
```{r etats}
quadrats <- read.csv2("BIO109/cours5/donnees/quadrats.txt")
...
```{r}
transMx <- read.table("BIO109/cours5/donnees/matrice_transitions.txt") 
etats <- c("B","M","T","R")
...
```{r fonction}
set.seed(123) # pour assurer la reproductibilité des résultats
...
simul.fun <- function(N, mx){
  for (i in 0:N){
      ...
      pasVecteur <- sapply(mx[[pas]], etat.fun) # tirage au sort
...
simulation <- simul.fun(10,etatsMx)
